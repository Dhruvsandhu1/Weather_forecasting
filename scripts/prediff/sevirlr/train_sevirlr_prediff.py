...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    def __init__(self,
        ...
        if pretrained_ckpt_path is not None:
            resolved_vae_path = os.path.join(default_pretrained_vae_dir, vae_cfg["pretrained_ckpt_path"])
            print(f"Using VAE checkpoint: {resolved_vae_path}")
            state_dict = torch.load(resolved_vae_path,
        ...
        if self.use_alignment:
            ...
            print(f"Using alignment checkpoint: {alignment_ckpt_path}")
    ...
    def get_trainer_config():
        ...
        cfg.process_group_backend = "auto"
        return cfg
    ...
    def set_trainer_kwargs(self, **kwargs):
        ...
        # Determine world size from provided devices/nodes
        ...
        world_size = max(1, num_devices * num_nodes)

        # Choose distributed backend
        pgb_cfg = getattr(self.oc.trainer, "process_group_backend", "auto")
        if pgb_cfg == "auto":
            use_nccl = torch.cuda.is_available() and platform.system() != "Windows"
            backend = "nccl" if use_nccl else "gloo"
        else:
            backend = pgb_cfg

        # Only set DDP strategy when world_size > 1
        strategy = None
        if world_size > 1:
            strategy = DDPStrategy(
                find_unused_parameters=self.oc.trainer.find_unused_parameters,
        ...
        ret = dict(
            ...
            # device accel
            accelerator="auto",
            # strategy is added below only if needed
    ...
    def save_vis_step_end(
        ...
        if isinstance(pred_seq, Sequence):
            ...
            label_list = [context_label, target_label, pred_label]
        if (not self.eval_example_only) or (data_idx in example_data_idx_list):
...
def get_parser():
    ...
    parser.add_argument('--ckpt', default=None, type=str,
                        help='The model checkpoint path to load weights before test.')
    parser.add_argument('--fast', action='store_true',
                        help='Run a quick pass with limited batches for faster sanity checks.')
    parser.add_argument('--limit_test_batches', type=int, default=None,
                        help='Override number of test batches to run (int).')
    return parser
...
def main():
    ...
    if args.limit_test_batches is not None:
        extra_trainer_kwargs['limit_test_batches'] = args.limit_test_batches
    ...
    if (args.pretrained or args.ckpt is not None) and not args.test:
        # load Earthformer-UNet
        if args.ckpt is not None:
            earthformerunet_ckpt_path = args.ckpt
        else:
            earthformerunet_ckpt_path = os.path.join(default_pretrained_earthformerunet_dir,
                                                    pretrained_sevirlr_earthformerunet_name)
        print(f"Using Earthformer-UNet checkpoint: {earthformerunet_ckpt_path}")
        ...
        if args.ckpt_name is not None:
            # Resolve checkpoint path: accept absolute, relative, or under save_dir/checkpoints
            candidate = args.ckpt_name
            if os.path.isabs(candidate) and os.path.exists(candidate):
                ckpt_path = candidate
            else:
                default_path = os.path.join(pl_module.save_dir, "checkpoints", candidate)
                ckpt_path = default_path if os.path.exists(default_path) else candidate
            if not os.path.exists(ckpt_path):
                raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")

            ext = os.path.splitext(ckpt_path)[1].lower()
            print(f"Using Earthformer-UNet checkpoint: {ckpt_path} ({'model-only' if ext in ('.pt', '.pth') else 'lightning'})")
            if ext in (".pt", ".pth"):
                # Model-only state_dict (no 'torch_nn_module.' prefix)
                state_dict = torch.load(ckpt_path, map_location=torch.device("cpu"))
                pl_module.torch_nn_module.load_state_dict(state_dict, strict=False)
            else:
                # Full PL checkpoint: extract model weights by prefix
                pl_ckpt = pl_load(path_or_url=ckpt_path, map_location=torch.device("cpu"))
